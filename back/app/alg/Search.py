...
def st_turnback(node1: Node, node2: Node) -> bool:
    if (
        node1.trans["harbor"].after == node2.trans["harbor"].befor and
        Time_lag(node1.trans["time"].after, node2.trans["time"].befor) <= 15
        and Time_lag(node1.trans["time"].after, node2.trans["time"].befor) > 0
        ):
        return True
    else:
        return False
...
if __name__ == "__main__":
    ...
    for i in [x for x, y in enumerate(test) if y != 1]:
        print(
            f'{i} {analyse_vec(net.A.T[i])} {analyse_vec(net.A[i])} {test[i]}'
            )
    for route in routes:
        node = Node()
        node.add_trans(
            "harbor",
            Trans(
                befor=net.nodes[route[0]].trans["harbor"].befor,
                after=net.nodes[route[-1]].trans["harbor"].after
                )
            )
        node.add_trans(
            "time",
            Trans(
                befor=net.nodes[route[0]].trans["time"].befor,
                after=net.nodes[route[-1]].trans["time"].after
                )
            )
    ...
    for r in routes:
        ...
        node.add_trans(
            "harbor",
            Trans(
                befor=nodes[r[0]].trans["harbor"].befor,
                after=nodes[r[-1]].trans["harbor"].after
                )
            )
        node.add_trans(
            "time",
            Trans(
                befor=nodes[r[0]].trans["time"].befor,
                after=nodes[r[-1]].trans["time"].after
                )
            )
    ...
    var = pulp.LpVariable.dicts(
        'VAR', (range(len(compressed_net.nodes)), range(Ship)), 0, 1, 'Binary'
        )
    ...
    for i in range(len(assigns)):
        table.append(
            [
                nodes[i].trans["time"].befor, nodes[i].trans["harbor"].befor,
                nodes[i].trans["time"].after, nodes[i].trans["harbor"].after,
                assigns[i]
                ]
            )
