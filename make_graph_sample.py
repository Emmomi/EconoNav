...
if __name__=='__main__':
    ...
    for n in range(len(Dh)):
        for i in range(len(Dh)):
            ...
            if (Ah[n]==Dh[i] and Time_lag(At[n],Dt[i])<=15 and Time_lag(At[n],Dt[i])>0):
        ...
        for j in period:
            ...
            if j==0:
                ...
                for n in j:
                    ...
                    if ((n[0]<=Dt[route[0]] and n[1]>=Dt[route[0]]) or (n[0]<=At[route[-1]] and n[1]>=At[route[-1]]))or((n[0]>=Dt[route[0]] and n[0]<=Dt[route[-1]]) or (n[1]>=Dt[route[0]] and n[1]<=At[route[-1]])):
                        break
                else:
